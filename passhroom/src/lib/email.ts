...
export async function sendMagicLinkEmail(input: SendMagicLinkInput): Promise<SendMagicLinkResult> {
  ...
  const subject = `Sign in to ${displayName}`;
  ...
  const text =
    ...
    `—\n` +
    `${env.smtp.from} is the email address used by Passhroom, the passwordless login system by fufroom that this website uses to make logins and accounts super easy because passwords suck!`;
  ...
  const buttonColor = '#B79AD0';
  const buttonHover = '#A983C8';
              ...
              <td style="background:#ffffff; border:1px solid #E6DDF0; border-radius:16px; padding:22px; box-shadow: 0 12px 32px rgba(45,13,60,0.08);">
                ...
                <div style="margin-top:18px;">
                  <a href="${safeMagicLinkUrl}"
                     style="display:inline-block; background:${buttonColor}; color:#ffffff; text-decoration:none; padding:12px 18px; border-radius:12px; font-weight:700; font-size:15px;">
                    Sign in
                  </a>
                </div>
            ...
            <tr>
              <td style="padding:14px 8px 0; font-size:11px; line-height:1.55; color:#7A6488;">
                <div style="margin-top:8px;">
                  ...
                  ${safeFromAddress} is the email address used by <strong style="color:#2D0D3C;">Passhroom</strong>, the passwordless login system by fufroom.
